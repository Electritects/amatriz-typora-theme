...
- Version: `v1.0.0`.
- Status: Final validation before publishing.
...
- During final validation, the installed Typora files are the working source of truth:
  - `%APPDATA%\Typora\themes\amatriz.css`.
  - `%APPDATA%\Typora\themes\amatriz-print-white.css`.
- Before publishing, copy the final working CSS files into:
  - `coding/amatriz.css`.
...
- The validation document uses manual HTML page breaks:

```html
<div style="page-break-after: always; break-after: page;"></div>
```

- These manual page breaks are for the validation/template document, not a requirement of the theme itself.
...
Current final-validation source of truth:

- `%APPDATA%\Typora\themes\amatriz.css`.
- `%APPDATA%\Typora\themes\amatriz-print-white.css`.

Temporary repo status:

- `coding/` is intentionally empty until the final CSS is accepted.

Before publishing, copy the final installed CSS into:
...
- Kept `coding/` empty during final validation by decision; copy final installed CSS there before publication.
